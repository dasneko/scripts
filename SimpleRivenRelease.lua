...
function TryAutoAttack(myTarget)
	return CheckAABoost() and myPlayer:Attack(myTarget)	
end
...
function CastQ(myTarget)	
	...
	if boxbox_ then
		if CheckAABoost() and ValidTarget(myTarget, myTrueRange) then
			...
			if useq_ and validT and SkillReady(skilllist[1]) and CanUseQ then										
				GetPosPredictionAndCast(myTarget, RivenTriCleave.delay, RivenTriCleave.speed, myPlayer, false, 2, skilllist[1])
				CanUseQ = false	
			end	
		end
	else
		if useq_ and validT and SkillReady(skilllist[1]) then										
			GetPosPredictionAndCast(myTarget, RivenTriCleave.delay, RivenTriCleave.speed, myPlayer, false, 2, skilllist[1])
		end
	end		
end
...
function CastW(myTarget)	
	...
	if boxbox_ then
		if CheckAABoost() and ValidTarget(myTarget, myTrueRange) then
			...
			if usew_ and validT and SkillReady(skilllist[2]) then			
...
function CastE(myTarget)	
	...
	if boxbox_ then
		if CheckAABoost() and ValidTarget(myTarget, myTrueRange) then
			...
			if usee_ and validT and SkillReady(skilllist[3]) then
				GetPosPredictionAndCast(myTarget, RivenFeint.delay, RivenFeint.speed, myPlayer, false, 2, skilllist[3])
				CanUseQ = false								
			end
		end
	else
	...
	if user_ and validT and SkillReady(skilllist[4]) and secondR then
		...
		if ultPer == 4 then ultPer_ = 75 end		
	 	if rTick + (GetLatency() *1000) < 10 or tHealth < ultPer_  then
			GetPosPredictionAndCast(myTarget, RivenFengShuiEngine.delay, RivenFeint.speed, myPlayer, false, 2, skilllist[4])
			secondR = false
		end
	end	
end
